...
static constexpr vPose velocity_smoothen(const vPose& vel_target,
                                         const vPose& vel_cur) {
  constexpr auto MAX_DIFF_LINEAR = MAX_VELOCITY_WHEEL_LINEAR * 0.03f;
...
static Pose pose_ctrl(const Pose& pose_sp, const Pose& pose_cur, float dt) {
  ...
  static constexpr float MAX_POSE_DEVIATION_ANGULAR = pi;
  static constexpr float K_P = 0.07;
  static constexpr float K_I = 0.001;
  static constexpr float K_D = 0.00001;
  ...
  integral.theta = std::clamp(static_cast<float>(integral.theta),
