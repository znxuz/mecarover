...
void rplidar_scan(rcl_timer_t* timer, int64_t last_call_time)
{
	 ...
	 * scan. in frame frame_id, angles are measured around the positive Z axis
	 ...
	 * points float32 scan_time        # time between scans [seconds] float32
	 * range_min        # minimum range value [m] float32 range_max        #
	 * maximum range value [m] float32[] ranges         # range data [m] (Note:
	 * values < range_min or > range_max should be discarded) float32[]
	 * intensities    # intensity data [device-specific units].  If your device
	 * does not provide intensities, please leave the array empty.
	 */
...
void micro_ros(void* controller)
{
	MX_LWIP_Init();
