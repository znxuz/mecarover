...
// static geometry_msgs__msg__Twist odometry_msg_buf{};
...
static void interpolation_cb(const void* arg) {
  WheelDataWrapper<real_t, WheelDataType::VEL_SP> wheel_sp_msg{
      std::array{1.3, 2.5, 3.7, 7.3}};
  static real_t counter = 1.2;
...
rclc_executor_t* interpolation_init(rcl_node_t* node, rclc_support_t* support,
  ...
  //                                &odometry_msg_buf, &pose_callback,
  //                                ON_NEW_DATA);
  ...
  rclc_executor_add_subscription(&interpolation_exe, &sub_cmd_vel,
                                 &cmd_vel_msg_buf, &interpolation_cb,
                                 ON_NEW_DATA);
