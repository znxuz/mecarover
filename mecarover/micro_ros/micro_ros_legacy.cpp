...
void rplidar_scan(rcl_timer_t* timer, int64_t last_call_time) {
  ...
  float endAngle =
      ((2 * M_PI) / requestedNumberOfPoints) * (requestedNumberOfPoints - 1);
...
void micro_ros_legacy(void* arg) {
  ...
  rcl_ret_check(rclc_publisher_init_default(
      &ctrl_status_pub, &node, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Byte),
      "ctrl_status"));
  rcl_ret_check(rclc_publisher_init_default(
  ...
  rcl_ret_check(rclc_executor_add_subscription(&executor, &sub_cmd_vel,
                                               &cmd_vel, &cmd_cb, ON_NEW_DATA));
  ...
  rcl_ret_check(rclc_executor_add_subscription(&lidar_executor, &sub_startScan,
